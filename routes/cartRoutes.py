...
def purchase_cart(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    purchased = cartController.purchase_cart(db, current_user.id)
    if not purchased:
        raise HTTPException(status_code=400, detail="No products available for purchase")
    return {"message": "Purchase successful", "products": purchased}